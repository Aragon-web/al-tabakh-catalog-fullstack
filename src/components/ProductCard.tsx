      ...
      >
        ...
        <div className="aspect-square relative overflow-hidden cursor-pointer" style={{ background: "var(--surface-2)" }} onClick={() => setShowDetail(true)}>
          ...
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors flex items-center justify-center">
            <span className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium bg-white/90 text-gray-800">
        ...
        <div className="p-3 space-y-1.5">
          <h3 className="text-sm font-semibold truncate cursor-pointer hover:underline" onClick={() => setShowDetail(true)}>{name}</h3>
